...
/* $Header: /cvs/sources/codecs/gsm/long_ter.cpp,v 1.1.1.1 2005/10/07 13:03:49 janis Exp $ */
...
# endif
		...
		*Nc_out = Nc;
		 ...
		 */
		for (bc = 0; bc <= 2; bc++) if (R <= gsm_mult(S, CGsmcodec::gsm_DLB[bc])) break;
		*bc_out = bc;
}
...
#				define	STEP_H(K)		STEP(K, h, a, b, c, d, e, f, g)
		...
		*Nc_out = Nc;
		...
		for (bc = 0; bc <= 2; ++bc) if (lambda <= CGsmcodec::gsm_DLB[bc]) break;
		*bc_out = bc;
}
