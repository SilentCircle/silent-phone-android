...
# Strip out files to publish that don't exist, to avoid
# failing the build when tar doesn't find the file(s).
strip_missing_files() {
    local filelist_file="$1"; shift
    local path
    ...
    while read path; do
        if [[ -r "$path" ]]; then
            ...
            printf "Missing file referenced in %s: %s\n" \
                "$filelist_file" "$path" >&2
        fi
    done
}

tarball="SilentPhone-iOS-doc_$(date +'%Y%m%d%H%M%S').tar.gz"
...
strip_missing_files "$FILES_TO_PUBLISH" |
while read path; do
