...
public class CallLogInteractionsLoader extends AsyncTaskLoader<List<ContactInteraction>> {
    ...
    private List<ContactInteraction> getCallLogInteractions(String phoneNumber) {
        final String normalizedNumber = PhoneNumberUtils.isGlobalPhoneNumber(phoneNumber)? 
                PhoneNumberHelper.normalizeNumber(phoneNumber) : phoneNumber;

        // If the number contains only symbols, we can skip it
        if (TextUtils.isEmpty(normalizedNumber)) {
            return Collections.emptyList();
        }
        final Uri uri = Uri.withAppendedPath(ScCalls.CONTENT_FILTER_URI,
        ...
        final String orderByAndLimit = ScCalls.DATE + " DESC LIMIT " + mMaxToRetrieve;
