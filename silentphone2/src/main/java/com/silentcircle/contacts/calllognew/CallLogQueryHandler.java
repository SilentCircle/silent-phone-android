...
public class CallLogQueryHandler extends NoNullCursorAsyncQueryHandler {
     ...
     * {@link android.database.sqlite.SQLiteException}, such as when the disk is full.
     */
...
//    }
    ...
    private void fetchCalls(int token, int callType, boolean newOnly, long newerThan) {
        ...
        if (callType > CALL_TYPE_ALL) {
            ...
            where.append(String.format("(%s = ?)", ScCalls.TYPE));
        ...
        if (newerThan > 0) {
            ...
            where.append(String.format("(%s > ?)", ScCalls.DATE));
        ...
        Uri uri = ScCalls.CONTENT_URI.buildUpon()
                .appendQueryParameter(ScCalls.LIMIT_PARAM_KEY, Integer.toString(limit))
                .build();
        startQuery(token, null, uri,
                CallLogQuery._PROJECTION, selection, selectionArgs.toArray(EMPTY_STRING_ARRAY), ScCalls.DEFAULT_SORT_ORDER);
    }
...
//        where.append(ScCalls.NEW);
//        where.append(" = 1 AND ");
//        where.append(ScCalls.TYPE);
//        where.append(" = ?");
//
//        ContentValues values = new ContentValues(1);
//        values.put(ScCalls.NEW, "0");
//
//        startUpdate(UPDATE_MARK_VOICEMAILS_AS_OLD_TOKEN, null, ScCalls.CONTENT_URI,
//                values, where.toString(), new String[]{ Integer.toString(ScCalls.VOICEMAIL_TYPE) });
//    }
    ...
    public interface Listener {
        /** Called when {@link com.silentcircle.contacts.calllognew.CallLogQueryHandler#fetchVoicemailStatus()} completes. */
        void onVoicemailStatusFetched(Cursor statusCursor);

        /**
         * Called when {@link com.silentcircle.contacts.calllognew.CallLogQueryHandler#fetchCalls(int)}complete.
