...
public class PhoneFavoritesTileAdapter extends BaseAdapter implements
    ...
    /**
     * Used when a contact is removed from speedDial. This will both un-star and set pinned position
     ...
     * particular position, return a list of {@link android.content.ContentProviderOperation}s that contains new
     ...
     */
    @VisibleForTesting
    ArrayList<ContentProviderOperation> getReflowedPinningOperations(
    ...
    public void onDragFinished(int x, int y) {
        setInDragging(false);
        // A contact has been dragged to the RemoveView in order to be un-starred,  so simply wait
        // for the new contact cursor which will cause the UI to be refreshed without the un-starred
        // contact.
        if (!mAwaitingRemove) {
            handleDrop();
        }
