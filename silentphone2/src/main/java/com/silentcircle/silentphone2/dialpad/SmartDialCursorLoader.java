...
public class SmartDialCursorLoader extends AsyncTaskLoader<Cursor> {
    ...
    private SmartDialMap mDialMap = new LatinSmartDialMap();
    ...
    public void configureQuery(String query) {
        ...
        for (int i = 0; i < query.length(); i++) {
            ...
            ch = mDialMap.normalizeCharacter(ch);
            if (mDialMap.isValidDialpadCharacter(ch)) {
                if (mDialMap.isValidDialpadAlphabeticChar(ch)) {
                    ch = mDialMap.getDialpadNumericCharacter(ch);
                }
                sb.append(ch);
            }
        }
        mQuery = sb.toString(); // SmartDialNameMatcher.normalizeNumber(query, SmartDialPrefix.getMap());
